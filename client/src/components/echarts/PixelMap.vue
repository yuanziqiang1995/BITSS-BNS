...
function renderItem(params, api) {
  ...
  let x =
    (api.value(0) + delta + (k / api.value(4)) * api.value(2)) * width + 0;
  // params.coordSys.x;
  let y =
    (api.value(1) + delta + (k / api.value(5)) * api.value(3)) * height + 0;
  // params.coordSys.y;
  width *= k / api.value(4);
  height *= k / api.value(5);
...
export default {
  ...
  methods: {
    showMap() {
      this.option.yAxis.data = this.param.condition;
      this.option.xAxis.data = this.param.noncondition;
