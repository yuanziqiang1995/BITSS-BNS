	...
	<div style="height: 100%;">
		<a-layout class="container">
			<a-layout-sider
				...
				<a-row>
					<a-checkable-tag v-model="tag.checked1" @change="toggleNodeShow1" class="tag">添加节点</a-checkable-tag>
					<div align="center">
						<a-list
			...
			<a-layout-sider
				...
				<a-form layout="horizontal" style="margin-top: 60px">
					...
					<a-form-item label="名称" :label-col="formItemLayout.labelCol" :wrapper-col="formItemLayout.wrapperCol">
						<a-input :value="currentSelect.nodeName" disabled />
					</a-form-item>
					<div v-if="currentParents.length">
						<a-divider dashed>证据</a-divider>		
						<div v-for="node in currentParents" v-bind:key="node.id" :value="node.id" >
							<a-form-item :label="node.nodeName" :label-col="formItemLayout.labelCol" :wrapper-col="formItemLayout.wrapperCol" >
								<!-- <a-select  v-for="node in currentParents" v-bind:key="node.id" :value="node.id"
									showSearch
									placeholder="Select a person"
									optionFilterProp="children"
									style="width: 200px">
										<a-select-option v-for="node in currentParents" v-bind:key="node.id" :value="node.id">
											{{node.name}}
										</a-select-option>
								</a-select> -->
								<a-radio-group v-model="node.evidence">
									<a-radio v-for="n in node.valueNum" :key="n" :value="n">{{n}}</a-radio>
								</a-radio-group>
							</a-form-item>
						</div>
						<div align="center">
						<a-button  type="primary" @click="infer">查询</a-button>
						</div>
					</div>
