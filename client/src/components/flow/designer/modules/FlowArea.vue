...
<script>
	...
	export default {
		props: ['browserType', 'flowData', 'plumb', 'select', 'selectGroup', 'parents', 'currentTool'],
		components: {
			jsplumb,
			FlowNode
		},
		...
		methods: {
			...
			flowInfo () {
				...
				alert('当前图中有 ' + nodeList.length + ' 个节点，有 ' + linkList.length + ' 条连线。');
			},
			paste () {
				const that = this;
				let dis = 0;
		...
		watch: {
			...
			currentSelect: {
				handler (val) {
					...
					let count=0;
					this.currentParents = [];	
					//console.log(this.flowData.linkList[0])
					for(let i=0; i<this.flowData.linkList.length; i++){
						//console.log(i);
						link = this.flowData.linkList[i];
						
						if(link.targetId == val.id){
						
							let node;
							for(let j=0; j<this.flowData.nodeList.length; j++){
								node = this.flowData.nodeList[j];
					...
					this.$emit('update:parents', this.currentParents);
			...
			// 		for(link in this.flowData.linkList){
			// 			console.log(link);
			// 			if(link.targetId == val.id){
			// 				let node;
			// 				for(node in this.nodeList){
			// 					if(node.id == this.flowData.targetId){
			// 						break;
			// 					}
			// 				}
			// 				count += 1;
