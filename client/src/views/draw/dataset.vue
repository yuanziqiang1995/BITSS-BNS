<template>
  <div class="app-container calendar-list-container">
    ...
    <div style="margin-bottom: 10px;margin-top: 0px">
      <el-button  size="small" style="" @click="uploadDataset()" type="primary" icon="el-icon-upload">上传数据集</el-button>
    </div>
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      style="width: 100%" :height="fullHeight-300"  @selection-change="handleDownloads">
      ...
      <el-table-column align="center" label="数据集描述"  >
        <template slot-scope="scope">
          <span>{{scope.row.description}}</span>
        </template>
      </el-table-column>
        <el-table-column align="center" label="数据集格式"  >
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="操作" width="400" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button  size="small" type="primary" @click="handleEdit(scope.row)" style='margin-left:10px'>编辑
          </el-button>
          <el-button  size="small" type="primary" @click="handleDownload(scope.row)" style='margin-left:10px'>下载
          </el-button>
          <el-button  size="small" type="danger" @click="handleDelete(scope.row)" style='margin-left:10px'>删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div class="pagination-container">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="listQuery.pages" :page-sizes="[10,20,30, 50]" :page-size="listQuery.rows" layout="total, sizes, prev, pager, next, jumper" :total="total">
      </el-pagination>
    </div>
    </div>
    <el-dialog title="数据基本信息" :visible.sync="dialogFormVisible"  v-dialogDrag >
      <el-form :rules="rules" ref="dataForm" :model="temp"  label-position="right" label-width="110px" style=' margin-left:50px;margin-right:50px; height:350px; '>
        ...
        <el-form-item label="数据描述 :" >
          <el-input v-model="temp.dataset.description" disabled type="textarea" :rows='2' style ='width:400px' maxlength='250'></el-input>
        </el-form-item>
...
export default {
      ...
      list: [
          {
            ...
            description: '11111',
            format:'.txt',
            updateDate: '2019-09-29 14:58:12'
          }, {    
            name: 'xxxx',
            description: '22222',
            format:'.txt',
            updateDate: '2019-11-29 14:58:12'
          }, {
            name: '123',
            description: '123',
            format:'.txt',
            ...
            description: '456',
            format:'.txt',
            updateDate: '2019-12-29 14:58:12'
          }
          ],
      total: 4,
      listLoading: true,
  ...
  methods: {
    ...
    handleDownload(row) {
      ...
      window.open('http://10.4.20.151:8101/govern-spark/hdfs/downloadFile?filePath='+row.position)
